...
const Page: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const formattedResponse = rawResponse
        ...
        .replace(/`(.*?)`/g, '<span class="italic bg-gray-200 px-1">$1</span>')
        .replace(/^\* (.*?)$/gm, '<ul><li class="list-disc">$1</li></ul>')
  ...
  return (
    <div className="flex flex-col lg:flex-row items-center justify-center w-full h-full bg-gradient-to-b from-blue-100 to-blue-300 p-4 gap-6">
      ...
      <div className="flex-1 w-full bg-white h-full p-6 rounded-2xl shadow-xl items-center mt-6 lg:mt-0">
        {loading ? (
          <div className="w-full text-center text-blue-500 font-semibold">Memproses pertanyaan Anda...</div>
        ) : response ? (
          <div className="mt-6 bg-gray-100 p-4 rounded-lg shadow overflow-y-auto max-h-[500px] w-full">
